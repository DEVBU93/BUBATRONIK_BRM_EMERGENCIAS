export default async function handler(req,res){
  ...
  try{
    ...
    const coords=b=>{const polygon=b.match(/<(?:(?:[\w.-]+):)?polygon\b[^>]*>([^<]+)<\/ /i)?.[1];if(polygon){const n=polygon.trim().split(/\s+/).map(Number).filter(Number.isFinite);if(n.length>=4){let a=0,o=0,c=0;for(let i=0;i+1<n.length;i+=2){a+=n[i];o+=n[i+1];c++}return{lat:a/c,lon:o/c}}}const p=b.match(/<(?:(?:[\w.-]+):)?point\b[^>]*>([^<]+)<\//i)?.[1]?.trim().split(/\s+/).map(Number);if(p?.length>=2&&p.slice(0,2).every(Number.isFinite))return{lat:p[0],lon:p[1]};const box=b.match(/<(?:(?:[\w.-]+):)?box\b[^>]*>([^<]+)<\//i)?.[1]?.trim().split(/\s+/).map(Number);if(box?.length>=4&&box.slice(0,4).every(Number.isFinite))return{lat:(box[0]+box[2])/2,lon:(box[1]+box[3])/2};return{lat:null,lon:null}};
